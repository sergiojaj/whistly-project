...
class CommentForm(forms.ModelForm):
    ...
    comment = forms.CharField(
        widget=forms.Textarea(
            attrs={"rows": 5, "placeholder": "Enter a beautiful comment here!"}
        ),
...
class ReplyForm(forms.ModelForm):
    ...
    reply = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 3, "placeholder": "Enter reply"}),
        max_length=600,
    )
...
class AddBirdForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "photographer_comment": forms.Textarea(
                attrs={"class": "form-control", "style": "height:80px mt-2"}
            ),
        }
